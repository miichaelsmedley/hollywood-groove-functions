...
export type DanceClaimResponseContract = {
  type?: string;
  action?: string;
  claimedAt?: number;
  displayName?: string;
...
export function isDanceClaimResponse(response: unknown): response is DanceClaimResponseContract {
  if (!isRecord(response)) {
    return false;
  }

  return response.type === "dance_claim" || response.action === "dance_claim";
}
